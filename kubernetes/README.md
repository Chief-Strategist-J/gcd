...
| **CLI Commands & Error Matrix** | [`shell-commands.md`](./shell-commands.md) | In-depth `kubectl` command manual, custom JSONPath projections, resource debugging (`logs`, `exec`, `debug`), and exhaustive Error Resolution Matrix (`CrashLoopBackOff`, `ImagePullBackOff`, `OOMKilled`, `Evicted`, `NodeNotReady`). |
...
| **Official References** | [`references.md`](./references.md) | Official Kubernetes documentation links, kubectl cheat sheet, API conventions, and CIS security benchmarks. |

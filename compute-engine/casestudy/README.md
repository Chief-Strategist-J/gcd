...
| [**6. Multi-Zone Internal Passthrough NLB Hands-On Blueprint**](file:///home/btpl-lap-22/live/gcd/compute-engine/casestudy/internal-passthrough-nlb-hands-on-lab.md) | Hands-On Lab & Operational Execution | Custom VPC (`my-internal-app`), Multi-Zone Subnets (`subnet-a`, `subnet-b`), Zero-Trust Ingress & Health Check Firewall Rules, Cloud Router & Cloud NAT Gateway (`nat-config`), Dual-Zone MIG Backends, Static Internal IP (`10.10.30.5`), and End-to-End Client CLI Testing (`utility-vm`). |
| [**7. Load Balancer Selection Decision Tree & Scheme Matrix**](file:///home/btpl-lap-22/live/gcd/compute-engine/casestudy/load-balancer-selection-decision-matrix.md) | Architectural Product Selection | Traffic Protocol Decision Trees (HTTP/S vs TCP/SSL vs UDP/Other), Client Access Scope (External vs Internal), Geographic Scope (Global vs Regional), Source IP Preservation vs Proxy Overhead, and Load-Balancing Schemes (`EXTERNAL`, `EXTERNAL_MANAGED`, `INTERNAL`, `INTERNAL_MANAGED`). |

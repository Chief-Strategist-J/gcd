...
| **CLI Commands & Error Matrix** | [`shell-commands.md`](./shell-commands.md) | Comprehensive `gcloud functions deploy` command manual, deep flag breakdown (`--gen2`, `--source`, `--entry-point`, `--runtime`), environment variables, Memorystore (Redis) VPC connectivity, Firestore Native mode triggers, Secret Manager volume mounting vs env vars, BigQuery Remote Functions CLI commands, and an exhaustive Troubleshooting Error Matrix. |

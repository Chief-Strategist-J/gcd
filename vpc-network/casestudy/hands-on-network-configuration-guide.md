...
#### 4. Create the Private VM Instance (No Public IP)
```bash
gcloud compute instances create vm-internal \
    --zone=us-central1-c \
    --machine-type=e2-standard-2 \
...
#### 6. Create the Cloud NAT Gateway with Telemetry & Connection Logging
```bash
gcloud compute routers nats create nat-config \
    --router=nat-router \
    --region=us-central1 \
...
#### 7. SSH to Private VM via IAP TCP Forwarding Tunnel
```bash
gcloud compute ssh vm-internal --zone=us-central1-c --tunnel-through-iap
```

#### 8. Inside VM — Test Outbound NAT Updates & Internet Egress
```bash
sudo apt-get update
```

#### 9. Inside VM — Test Cloud Storage Access via Private Google Access (PGA)
```bash
gcloud storage ls
```

#### 10. Exit VM & Read Cloud NAT Gateway Logs in Cloud Logging
```bash
exit
